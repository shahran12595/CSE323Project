...
// Allocate a zeroed disk block. // FFS Version updated for Block Groups**
static uint
balloc(uint dev)
{
  int g, bi, m;
  ...
  for(g = 0; g < sb.ngroups; g++) {
    uint group_start = global_start + (g * bpg);
    ...
    for(bi = 0; bi < BPB && bi < bpg; bi++) {
      ...
      if((bp->data[bi/8] & m) == 0){  // Is bit zero (free)?
        ...
        // --- [FIX START] Correctly zero the disk block ---
...
// Free a disk block. updated for Block Groups**
static void
bfree(int dev, uint b)
{
  struct buf *bp;
  ...
  // 1. Calculate the start of the managed area (where groups begin)
  uint global_start = sb.logstart + sb.nlog;

  // 2. Find the bitmap block (BBLOCK macro handles the block address)
  bp = bread(dev, BBLOCK(b, sb));

  // 3. Calculate the bit index within that group's bitmap
  // OLD LOGIC: bi = b % BPB;  <-- INCORRECT for Block Groups
  // NEW LOGIC: Relative offset within the group
